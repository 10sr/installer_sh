...
_carch=`uname -m`
if test "$_carch" = "i686" || test "$_carch" = "i386"
then
    _carch=i386
elif test "$_carch" = "x86_64"
then
    _carch=amd64
else
    echo "$_carch: Unknown archtecture"
    exit 1
fi
...
main(){
    cd $srcdir/ && \
        install -Dm 755 $_pkgname-$_carch $_prefix/bin/$_pkgname && \
        install -d $_prefix/share/man/man1 && \
        ./$_pkgname-$_carch --manpage | gzip >$_prefix/share/man/man1/$_pkgname.1.gz
}
