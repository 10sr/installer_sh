...
pkgname=sdcv-dict
pkgver=1.0
pkgdesc="Dictionary files for stardict/sdcv"
url="http://abloz.com/huzheng/stardict-dic"
_dict_dict_org=stardict-dictd_www.dict.org_gcide-2.4.2
_dict_jmdict=stardict-jmdict-en-ja-2.4.2
_dict_gene95=stardict-ej-gene95-2.4.2
source="$url/dict.org/$_dict_dict_org.tar.bz2
$url/ja/$_dict_jmdict.tar.bz2
$url/ja/$_dict_gene95.tar.bz2"

_stardict_dir="$HOME/.stardict/dic"

main(){
    mkdir -p "$_stardict_dir" || return 1
    for dir in $_dict_dict_org $_dict_jmdict $_dict_gene95
    do
        cp -r "$srcdir/$dir" "$_stardict_dir" || return 1
    done
}
