...
pkgname=emacs
pkgver=24.3
pkgdesc="Editor (without X)"
url="http://www.gnu.org/software/$pkgname"
source="http://core.ring.gr.jp/pub/GNU/$pkgname/$pkgname-$pkgver.tar.xz"

main(){
    test "$1" = pass && return 1
    cd $srcdir/$pkgname-$pkgver && \
        ./configure --prefix="$HOME/.local" --without-x --without-sound && \
        make && \
        make check && \
        make install
}
...
################################################################################
# emacs24-installer.sh --- Install Emacs without X
